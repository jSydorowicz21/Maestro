 ...
 *
 * React hook for accessing Layer 1 (static) agent capabilities.
 * Provides type-safe access to what features each agent *type* supports,
 * independent of any running session.
 *
 * For Layer 2 (harness runtime capabilities) and Layer 3 (session runtime
 * metadata), see useSessionRuntimeCapabilities.ts and harnessStore.ts.
 */

import { useState, useEffect, useCallback } from 'react';
import type { ToolType } from '../../types';

// Import the shared type — single source of truth for all processes
