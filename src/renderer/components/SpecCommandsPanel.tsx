/**
 * Shared Spec Commands Panel
 *
 * Base UI component for displaying and managing spec command systems
 * (SpecKit, OpenSpec). Each feature provides its own config to customize
 * labels, icons, IPC namespace, and descriptions.
 */
...
import type { Theme, EditingCommand } from '../types';
...
// IPC namespace shape shared by speckit, openspec, and bmad
export interface SpecCommandsIPC {
	getMetadata: () => Promise<{
		success: boolean;
...
export interface SpecCommandsPanelConfig {
	icon: LucideIcon;
	label: string;
...
interface SpecCommandsPanelProps {
	theme: Theme;
	config: SpecCommandsPanelConfig;
}

export function SpecCommandsPanel({ theme, config }: SpecCommandsPanelProps) {
	const {
		icon: Icon,
		label,
		descriptionPrefix,
	...
	const [commands, setCommands] = useState<SpecCommand[]>([]);
	const [metadata, setMetadata] = useState<SpecCommandMetadata | null>(null);
	...
	useEffect(() => {
		const loadData = async () => {
			...
			try {
				...
				// Expected: spec command operations may fail due to file system errors
				console.error(`Failed to load ${logPrefix} commands:`, error);
			} finally {
				setIsLoading(false);
			}
		};
	...
	const handleSaveEdit = async () => {
		...
		try {
			const result = await ipc.savePrompt(editingCommand.id, editingCommand.prompt);
			if (result.success) {
				setCommands(
					commands.map((cmd) =>
			...
			// Expected: spec command operations may fail due to file system errors
			console.error(`Failed to save ${logPrefix} prompt:`, error);
		}
	};

	const handleReset = async (id: string) => {
		try {
			const result = await ipc.resetPrompt(id);
			...
			// Expected: spec command operations may fail due to file system errors
			console.error(`Failed to reset ${logPrefix} prompt:`, error);
		}
	};
	...
	const handleRefresh = async () => {
		...
		try {
			const result = await ipc.refresh();
			if (result.success && result.metadata) {
				setMetadata(result.metadata);
				// Reload prompts after refresh
				const promptsResult = await ipc.getPrompts();
				if (promptsResult.success && promptsResult.commands) {
					setCommands(promptsResult.commands);
				}
			}
			...
			// Expected: spec command operations may fail due to file system errors
			console.error(`Failed to refresh ${logPrefix} prompts:`, error);
		} finally {
		...
		<div className="space-y-4">
			<div>
				...
				<p className="text-xs opacity-50" style={{ color: theme.colors.textDim }}>
					{descriptionPrefix}{' '}
					<button
						onClick={() => window.maestro.shell.openExternal(externalUrl)}
