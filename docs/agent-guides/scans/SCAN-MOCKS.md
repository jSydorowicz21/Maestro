...
- `createMockTab` / `createMockAITab`: **20 defining files** (was 12, +8)
- Files with `window.maestro = {` setup: **70 files** (previous scan reported "117 instances"; re-counted by unique files)

Consolidation target (`src/__tests__/helpers/`) has not been created. Individual file:line numbers below are NOT re-verified in this refresh pass.
