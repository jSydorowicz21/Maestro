...
**Refreshed 2026-04-10 against origin/rc (06e5a2eb3)**: `src/renderer/components/ui/` currently contains only `EmojiPickerField.tsx`, `FormInput.tsx`, `Modal.tsx`, `ToggleSwitch.tsx`. The proposed consolidation targets `EmptyState`, `GhostIconButton`, and `Spinner` have NOT been implemented, so all duplication surfaces below are still live. Individual file:line numbers NOT re-verified in this refresh pass.
